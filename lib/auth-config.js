...
/** Approved bcrypt cost values (canonical decimal strings only) */
const APPROVED_BCRYPT_COSTS = new Set(["12", "13", "14"]);

/**
 * Resolves and validates BCRYPT_COST from the environment.
 *
 * Accepts only the exact strings '12', '13', or '14' from the BCRYPT_COST
 * env var. No sign, fraction, suffix, prefix, or surrounding whitespace is
 * permitted. Defaults to 12 when BCRYPT_COST is not set (undefined).
 * Terminates the process with exit code 1 for any invalid value.
 *
 * @param {object} env - Environment object (typically process.env)
 * @returns {number} The validated bcrypt cost as an integer (12, 13, or 14)
 */
function resolveBcryptCost(env) {
  const raw = env.BCRYPT_COST;

  // Not configured → default to cost 12
  if (raw === undefined) {
    return 12;
  }

  // Must be exactly '12', '13', or '14' — canonical decimal only
  if (!APPROVED_BCRYPT_COSTS.has(raw)) {
    console.error(
      "[auth-config] FATAL: BCRYPT_COST must be exactly '12', '13', or '14' (canonical decimal). " +
        "No sign, fraction, suffix, prefix, or whitespace is accepted. Terminating."
    );
    process.exit(1);
  }
...
/**
 * Generates a dummy bcrypt hash at the specified cost.
 *
 * This hash is used for timing equalization when no real user hash is available,
 * ensuring credential verification always performs exactly one bcrypt comparison
 * regardless of whether the account exists.
 *
 * @param {number} cost - The bcrypt cost factor (12, 13, or 14)
 * @returns {string} A bcrypt hash string at the configured cost
 */
function generateDummyHash(cost) {
  return bcrypt.hashSync("dummy-password-for-timing-equalization", cost);
}
...
/** Canonical decimal integer pattern: no sign, no fraction, no whitespace, no suffix */
const CANONICAL_INT_PATTERN = /^[0-9]+$/;

/**
 * Resolves and validates JWT_LIFETIME from the environment.
 *
 * Accepts only a canonical decimal integer from 1 through 3600 (seconds).
 * No sign, fraction, suffix, prefix, or surrounding whitespace is permitted.
 * Defaults to 900 seconds when JWT_LIFETIME is not set (undefined).
 * Terminates the process with exit code 1 for any invalid value.
 *
 * @param {object} env - Environment object (typically process.env)
 * @returns {number} The validated JWT lifetime in seconds (1–3600)
 */
function resolveJwtLifetime(env) {
  const raw = env.JWT_LIFETIME;

  // Not configured → default to 900 seconds
  if (raw === undefined) {
    return 900;
  }

  // Must be a canonical decimal integer (no sign, no fraction, no whitespace, no suffix)
  if (!CANONICAL_INT_PATTERN.test(raw)) {
    console.error(
      "[auth-config] FATAL: JWT_LIFETIME must be a canonical decimal integer (1-3600). " +
        "No sign, fraction, suffix, prefix, or whitespace is accepted. Terminating."
    );
    process.exit(1);
  }
  ...
  // Must be within 1–3600 range
  if (value < 1 || value > 3600) {
    console.error(
      "[auth-config] FATAL: JWT_LIFETIME must be between 1 and 3600 seconds inclusive. " +
        "Got " + value + ". Terminating."
    );
    process.exit(1);
  }
...
module.exports = {
  ...
  _internals: {
    ...
    PROHIBITED_SECRETS,
    APPROVED_BCRYPT_COSTS,
    CANONICAL_INT_PATTERN,
    isValidHex,
    isValidBase64,
  },
};
