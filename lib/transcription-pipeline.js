 ...
 *   { rawText, correctedText, entities, wordTimings, corrections, rawWords,
 *     provider, durationS, correlationId }
 */
async function transcribeStoredAudio(audioPath, options = {}) {
  ...
  return {
    ...
    // Per-change corrections and the raw ASR words the pipeline already
    // produces. They ride alongside the result so completeTranscription can
    // persist them into the transcript row's own `metadata` column, where the
    // Python ingestion reads `metadata.corrections` / `metadata.raw_words` to
    // build correction evidence. Absent them, the corrections array would be
    // dropped here and evidence could never flow end to end
    // (transcript-evidence-navigation Req 8.5/8.6: no new table, no new write —
    // metadata is a column on the transcript row Python owns reading of).
    corrections: formatted.corrections || [],
    rawWords: rawWords.map((w) => ({ ...w })),
