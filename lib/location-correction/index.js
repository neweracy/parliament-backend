...
const SUPPLEMENTARY_LOCATIONS = [
  ...
  { canonical: 'Ningo', region: 'Greater Accra', aliases: ['New Ningo', 'Old Ningo'] },
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (isTitle(tokens[i].word)) {
      ...
      if (titleResult) {
        ...
        const isIdentity = match.canonical.toLowerCase() === original.toLowerCase();
...
/**
 * Corrects location names in a word-level segments array.
 * Each segment has a `.text` (plus optional `.start`, `.end`, `.confidence`).
 *
 * @param {Array<{ text: string }>} segments
 * @param {object} [options]
 * @returns {{ segments: Array, corrections: Array }}
 */
function correctSegments(segments, options = {}) {
  const allCorrections = [];
  const correctedSegments = segments.map((seg) => {
    if (!seg.text || seg.text.length < 3) return seg;
    const result = correctLocations(seg.text, options);
